...
export function NetworkFooter() {
  return (
    <div className="lb-network">
      <div className="lb-network-grid">
        {networkSites.map((site) => (
          <a
            key={site.href}
            href={site.href}
            title={site.title}
            className={`lb-network-link${site.active ? " lb-active" : ""}`}
            {...(!site.active && {
              target: "_blank",
              rel: "noopener noreferrer",
            })}
          >
            <span className="lb-network-icon">{site.icon}</span>
            {site.label}
          </a>
        ))}
      </div>
      ...
      <style>{`
        ...
        .lb-network-link:hover {
          color: rgba(255, 255, 255, 0.65);
        }
        .lb-network-link.lb-active {
