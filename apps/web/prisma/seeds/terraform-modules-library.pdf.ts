...
const ACCENT = rgb(0.0, 0.32, 0.78);
const MUTED = rgb(0.42, 0.45, 0.5);
const CODE_BG = rgb(0.96, 0.97, 0.99);
const CODE_FG = rgb(0.12, 0.14, 0.18);
...
function drawPageChrome(ctx: DocCtx): void {
  if (ctx.pageNo <= 1) return;
  // Header band
  ctx.page.drawText("Terraform Modules Library", {
    x: MARGIN_X,
    y: PAGE_H - 36,
    size: 9,
    font: ctx.body,
  ...
  ctx.page.drawText("CopyPasteLearn · Open Empower B.V.", {
    x: PAGE_W - MARGIN_X - ctx.body.widthOfTextAtSize("CopyPasteLearn · Open Empower B.V.", 9),
    y: PAGE_H - 36,
    size: 9,
    font: ctx.body,
    color: MUTED,
...
function drawHeading(ctx: DocCtx, text: string, level: 1 | 2 | 3): void {
...
function drawParagraph(ctx: DocCtx, text: string): void {
  const size = 10.5;
  const lineH = 14;
...
function drawCode(ctx: DocCtx, code: string): void {
  const size = 9;
  const lineH = 12;
  const padX = 10;
  const padY = 8;
  ...
  for (const raw of code.split("\n")) {
    if (ctx.mono.widthOfTextAtSize(raw, size) <= maxW) {
      ...
      // Hard-wrap long code lines at character boundaries
      let buf = "";
      for (const ch of raw) {
...
function drawCover(ctx: DocCtx): void {
  // ctx.page is the first page (created in main); chrome skipped on cover
  ctx.page.drawRectangle({
    x: 0,
    y: PAGE_H - 200,
    width: PAGE_W,
  ...
  ctx.page.drawText("Battle-tested Terraform modules for AWS, GCP, and Azure", {
    x: MARGIN_X,
    y: PAGE_H - 170,
    size: 12,
    font: ctx.body,
  ...
  ctx.page.drawText("Version 1.1 · May 2026", {
    x: MARGIN_X,
    y: 130,
    size: 11,
    font: ctx.body,
...
function drawToc(ctx: DocCtx, entries: string[]): void {
...
export async function generateTerraformModulesLibraryPdf(): Promise<Uint8Array> {
  ...
  pdf.setTitle("Terraform Modules Library");
  pdf.setAuthor("Open Empower B.V. (CopyPasteLearn)");
  pdf.setSubject("Battle-tested Terraform modules for AWS, GCP, and Azure.");
  pdf.setKeywords([
    "terraform",
    "iac",
    "aws",
