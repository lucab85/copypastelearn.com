...
Every file, directory, and socket has a security context:
...
The **type** field (`httpd_sys_content_t`) is what matters most. SELinux policy uses types to decide which processes can access which files.

## The Labeling Problem

When you create files in a non-standard location, they inherit the **parent directory's** label — which is usually wrong:
...
Apache (`httpd_t`) needs `httpd_sys_content_t` but gets `var_t`. Result: **403 Forbidden** even though DAC permissions are fine.

## Quick Fix: chcon

`chcon` changes contexts immediately but **does not persist**:

```
chcon -t httpd_sys_content_t -R /srv/webroot
```

This works instantly, but `restorecon` or a relabel will overwrite your changes. Use `chcon` for testing, not production.
...
New files created under `/srv/webroot` will now automatically get the correct label.

## Finding the Right Labels

Not sure which label to use? SELinux policy already defines where files should go:

```
semanage fcontext -l | grep cert_t
```

This shows all paths that expect certificate files — useful for knowing where to put TLS certs, for example.
...
Always use `semanage fcontext` + `restorecon` for persistent changes. The workflow:

1. `matchpathcon` — check current expected context
2. `semanage fcontext` — define the mapping
3. `restorecon` — apply it
4. Verify with `ls -Z`

Practice this workflow hands-on in our [SELinux for System Admins course](/courses) with real RHEL 9/10 labs.
