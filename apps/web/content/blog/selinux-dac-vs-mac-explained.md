...
DAC is the traditional Unix permission model you already know:
...
The problem? If an attacker compromises a process running as a user, they inherit **all** that user's permissions. And if they get root, game over.
...
Notice the `-Z` flag — it reveals the SELinux **security context**: `system_u:object_r:passwd_file_t:s0`.
...
Type Enforcement (TE) is the core decision model. The policy defines which **types** (domains) can interact with which other types:

- `httpd_t` can read files labeled `httpd_sys_content_t`
- `httpd_t` **cannot** read files labeled `var_t`
- Even if DAC says `chmod 777`

This is why relabeling files correctly matters more than file permissions in an SELinux environment.
